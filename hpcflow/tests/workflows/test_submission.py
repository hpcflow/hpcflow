...
def test_zarr_metadata_file_modification_times_many_jobscripts(null_config, tmp_path):
    ...
    t1 = hf.Task(
        ...
        inputs={"p1": 100},
        sequences=[
            hf.ValueSequence(path="resources.any.resources_id", values=list(range(10)))
        ],
    )
    ...
    mtime_jobscript0_chunk = (
        Path(wk._store.path)
        .joinpath(wk._store._get_jobscripts_metadata_arr(0).path, "0")
        .stat()
        .st_mtime
    )
    ...
    assert mtime_meta_group < mtime_jobscript0_chunk < mtime_submission_group
